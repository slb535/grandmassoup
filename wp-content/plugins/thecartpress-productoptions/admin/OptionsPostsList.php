...
<div class="wrap">
<h2><?php _e( 'Options Posts Update', 'tcp_po' );?></h2>
<div class="clear"></div>

<form method="post">
	<p class="search-box">
	<label for="post_type"><?php _e( 'Post type', 'tcp_po' )?>:</label>
	...
	<input type="submit" name="tcp_load_taxonomies" value="<?php _e( 'Load taxonomies', 'tcp_po' );?>" class="button-secondary"/>

	<label for="taxonomy"><?php _e( 'Taxonomy', 'tcp_po' )?>:</label>
	...
	<input type="submit" name="tcp_load_terms" value="<?php _e( 'Load categories', 'tcp_po' );?>" class="button-secondary"/>
	<label for="category_slug"><?php _e( 'Category', 'tcp_po' );?>:</label>
	<select id="category_slug" name="category_slug">
		<option value="0"><?php _e( 'no one selected', 'tcp_po' );?></option>
	...
	<input type="submit" name="tcp_search" value="<?php _e( 'Search', 'tcp_po' );?>" class="button-secondary"/>	
	</p>
	<?php if ( isset( $_REQUEST['tcp_search'] ) && strlen( $cat_slug ) > 0 ) :
		...
		if ( is_array( $posts_with_options ) && count( $posts_with_options ) ) {
			...
			$html .= '<option value="" select="true">' . __( 'No change', 'tcp_po' ) . '</option>';
		...
		<div>
			...
			<thead>
			<tr>
				<th scope="col" class="manage-column"><?php _e( 'Title', 'tcp_po' );?></th>
				<th scope="col" class="manage-column"><?php _e( 'Label', 'tcp_po' );?></th>
			...
			<tr>
				...
				<td>
					...
					<label for="tcp_create_option"><?php _e( 'No create options', 'tcp_po' );?></label>:
					...
					<?php if ( $first_row ) : $first_row = false;?>
						<input type="button" class="button-secondary" value="<?php _e( 'Copy to all', 'tcp_po' );?>" onclick="copy_to_all(<?php echo $id;?>);"/>
					<?php endif;?>
				</td>
			</tr>
			<?php endforeach;?>
		...
		<?php else : ?>
		<p><?php _e( 'No products to update', 'tcp_po' );?></p>
		<?php endif;?>
	<p class="submit">
		<input type="submit" id="tcp_options_posts_update" name="tcp_options_posts_update" class="button-primary" value="<?php _e( 'Update' ) ?>" />
	</p>
	<?php else : ?>
	<p><?php _e( 'First, you must perform a search:', 'tcp_po' );?></p>
	<?php endif;?>
</form>
