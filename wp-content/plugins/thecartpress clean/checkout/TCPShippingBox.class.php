...
class TCPShippingBox extends TCPCheckoutBox {
	...
	function get_title() {
		return __( 'Delivery options', 'tcp' );
	}
	...
	function after_action() {
		...
		if ( $selected_shipping_address == 'new' ) {
			if ( ! isset( $_REQUEST['shipping_firstname'] ) || strlen( $_REQUEST['shipping_firstname'] ) == 0 )
				$this->errors['shipping_firstname'] = __( 'The Delivery First name field must be completed', 'tcp' );
			if ( ! isset( $_REQUEST['shipping_lastname'] ) || strlen( $_REQUEST['shipping_lastname'] ) == 0 )
				$this->errors['shipping_lastname'] = __( 'The Delivery Last name field must be completed', 'tcp' );
			if ( ! isset( $_REQUEST['shipping_street'] ) || strlen( $_REQUEST['shipping_street'] ) == 0 )
				$this->errors['shipping_street'] = __( 'The Delivery Street field must be completed', 'tcp' );
			if ( isset( $_REQUEST['shipping_city'] ) && strlen( $_REQUEST['shipping_city'] ) == 0 )
				$this->errors['shipping_city'] = __( 'The Delivery City field must be completed', 'tcp' );
			if ( isset( $_REQUEST['shipping_region'] ) && strlen( $_REQUEST['shipping_region'] ) == 0 && $_REQUEST['shipping_region_id'] == '' )
				$this->errors['shipping_region'] = __( 'The Delivery Region field must be completed', 'tcp' );
			global $thecartpress;
			$shipping_isos = $thecartpress->get_setting( 'shipping_isos', false );
			if ( $shipping_isos ) {
				if ( ! in_array( $_REQUEST['shipping_country_id'], $shipping_isos ) ) {
					$this->errors['shipping_country_id'] = __( 'The Delivery Country is not allowed', 'tcp' );
				}
			}
			if ( ! isset( $_REQUEST['shipping_postcode'] ) || strlen( $_REQUEST['shipping_postcode'] ) == 0 )
				$this->errors['shipping_postcode'] = __( 'The delivery postal code field must be completed', 'tcp' );
			if ( ! isset( $_REQUEST['shipping_email'] ) || strlen( $_REQUEST['shipping_email'] ) == 0 )
				$this->errors['shipping_email'] = __( 'The delivery eMail field must be completed', 'tcp' );
			elseif ( ! $this->check_email_address( $_REQUEST['shipping_email'] ) ) 
				$this->errors['shipping_email'] = __( 'The delivery eMail field must be a valid email', 'tcp' );
...
//			$selected_shipping_address = 'new';
	...
	function show() {
		...
		if ( count( $addresses ) > 0 ) {
			...
			<label for="selected_shipping_address"><?php _e( 'delivery to the address selected', 'tcp' )?></label>
			<br /><?php
		} ?>
			...
			<label for="new_shipping_address"><?php _e( 'New delivery address', 'tcp' );?></label>
			...
			if ( isset( $_REQUEST['shipping_email'] ) ) {
				...
				<ul>
					...
					<li><label for="shipping_company"><?php _e( 'Meal Contact Name', 'tcp' );?>:</label>
					...
					<li><label for="shipping_tax_id_number"><?php _e( 'Meal Contact Cell', 'tcp' );?>:</label>
