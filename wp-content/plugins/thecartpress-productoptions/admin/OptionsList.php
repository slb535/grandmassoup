...
if ( $index !== false ) {
	...
	if ( $post_id_to > 0 ) {
		...
		<div id="message" class="updated"><p>
			<?php _e( 'Relation created', 'tcp_po' );?>
		</p></div><?php
	}
}
...
//} elseif ( isset( $_REQUEST['tcp_delete_relation'] ) ) {
	...
	if ( $post_id_delete > 0 && RelEntities::count( $post_id_to, $rel_type ) == 0 ) {
		...
		<div id="message" class="updated"><p>
			<?php _e( 'Relation deleted', 'tcp_po' );?>
		</p></div><?php
	}
}
...
//} elseif ( isset( $_REQUEST['tcp_delete_children'] ) ) {
	...
	if ( $post_id_delete > 0 && RelEntities::count( $post_id_delete, $rel_type ) > 0 ) {
		...
		<div id="message" class="updated"><p>
			<?php _e( 'Children deleted', 'tcp_po' );?>
		</p></div><?php
	}
}
...
if ( $index !== false ) {
	...
	update_post_meta( $option_id_to, 'tcp_order', $order );?>
	<div id="message" class="updated"><p>
		<?php _e( 'Price and Order updated', 'tcp_po' );?>
	</p></div><?php
}
...
<div class="wrap">
	...
	<h2><?php echo __( 'Options for', 'tcp_po' ), '&nbsp;', $post->post_title;?></h2>
	<ul class="subsubsub">
		<li><a href="post.php?action=edit&post=<?php echo $post->ID;?>"><?php _e( 'return to the product', 'tcp_po' );?></a></li>
		<li>|</li>
		<li><a href="post-new.php?post_type=tcp_product_option&tcp_product_parent_id=<?php echo $post->ID;?>"><?php _e( 'create new option', 'tcp_po' );?></a></li>
	</ul>
	<div class="clear"></div>
...
<form name="frm" method="post">
	...
	<p class="search-box">
		<label for="category_id"><?php _e( 'Category', 'tcp_po' );?>:</label>
		<select id="category_slug" name="category_slug">
			<option value="0" selected="selected"><?php _e( 'no one selected', 'tcp_po' );?></option>
		...
		<input id="tcp_filter_by_category" name="tcp_filter_by_category" value="<?php _e( 'filter', 'tcp_po' );?>" type="submit" class="button-secondary" />

		<label for="product_id_from"><?php _e( 'product', 'tcp_po' );?>:</label>
		<select id="post_id_from" name="post_id_from">
			<?php if ( strlen( $cat_slug ) > 0 ) :
				...
				$ids = get_posts( $args );
				if ( is_array( $ids ) && count( $ids ) > 0 ) : foreach( $ids as $id ) :
					if ( $id != $post_id && RelEntities::count( $id, 'OPTIONS' ) > 0 ) : ?>
			<option value="<?php echo $id; ?>" <?php selected( $id, isset( $_REQUEST['post_id_from'] ) ? $_REQUEST['post_id_from'] : 0 );?>><?php echo get_the_title( $id ); ?></option>
					<?php endif;?>
				<?php endforeach;
				endif;
			else : ?>
			<option value="" selected="selected"><?php _e( 'Please, filter first a category', 'tcp_po' );?></option>
			<?php endif;?>
		</select>
		<input id="tcp_copy_options_from_product" name="tcp_copy_options_from_product" value="<?php _e( 'copy', 'tcp_po' );?>" type="submit" class="button-secondary" />
	</p>
</form>

<form method="post">
<p><input type="submit" id="tcp_update_price" name="tcp_update_price" value="<?php _e( 'modify all', 'tcp_po' );?>" class="button-primary"/></p>
<table class="widefat fixed">
<thead>
<tr>
	<th scope="col" class="manage-column"><?php _e( 'Thumbnail', 'tcp_po' );?></th>
	<th scope="col" class="manage-column"><?php _e( 'Option', 'tcp_po' );?></th>
	<th scope="col" class="manage-column"><?php _e( 'Description', 'tcp_po' );?></th>
	<th scope="col" class="manage-column"><?php _e( 'Status', 'tcp_po' );?></th>
	<th scope="col" class="manage-column" style="width: 25%;"><?php _e( 'Price and order', 'tcp_po' );?></th>
...
if ( is_array( $options ) && count( $options ) > 0 ) :
	foreach( $options as $i => $option ) : $post = get_post( $option->id_to ); 
		if ( $post ) : ?>
	<tr>
		<td><a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post->ID;?>" title="<?php _e( 'edit option', 'tcp_po' ); ?>"><?php echo tcp_get_the_thumbnail( $post_id, $post->ID, 0, array( '50', '50' ) ); ?></a></td>
		<td><a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post->ID;?>" title="<?php _e( 'edit option', 'tcp_po' ); ?>"><?php echo $post->post_title;?></a></td>
		...
		<td>
			...
			&nbsp;<label><?php echo __( 'Order', 'tcp_po' );?>:&nbsp;<input type="text" id="tcp_order" name="tcp_order_<?php echo $i;?>" value="<?php echo tcp_get_the_order( $post->ID );?>" size="4" maxlength="8" class="tcp_count"/></label>
		</td>
		<td>
			<a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post->ID;?>"><?php _e( 'edit option', 'tcp_po' );?></a>
			&nbsp;|&nbsp;
		<?php if ( RelEntities::count( $option->id_to, $rel_type ) == 0 ) :?>
			<a href="#" onclick="return show_delete_relation(<?php echo $post->ID;?>);" class="delete"><?php _e( 'delete option', 'tcp_po' );?></a>
		<?php else:?>
			<a href="#" onclick="return show_delete_children(<?php echo $post->ID;?>);" class="delete"><?php _e( 'delete children', 'tcp_po' );?></a>			
		<?php endif;?>
			&nbsp;|&nbsp;
			<a href="post-new.php?post_type=tcp_product_option&tcp_product_parent_id=<?php echo $post_id;?>&tcp_product_option_parent_id=<?php echo $post->ID;?>"><?php _e( 'create child option', 'tcp_po' );?></a>
			<br/>
			<div class="wrap copy_relation">
				...
				<input id="tcp_copy_suboptions" name="tcp_copy_suboptions_<?php echo $i;?>" value="<?php _e( 'copy from' , 'tcp_po' );?>" type="submit" class="button-secondary"/>
			...
			<?php if ( RelEntities::count( $option->id_to, $rel_type ) == 0 ) :?>
				<div class="wrap delete_relation" id="div_delete_relation_<?php echo $option->id_to;?>" style="display: none; border: 1px dotted orange; padding: 2px">
					...
					<input id="rel_type" name="rel_type" value="<?php echo $rel_type;?>" type="hidden" />
					<p><?php _e( 'Do you really want to delete this option?', 'tcp_po' );?></p>
					<input type="submit" id="tcp_delete_relation" name="tcp_delete_relation_<?php echo $i;?>" class="button-secondary" value="<?php _e( 'Yes' , 'tcp_po' );?>"/> |
					<a href="#" onclick="jQuery('#div_delete_relation_<?php echo $option->id_to;?>').hide();return false;"><?php _e( 'No, I don\'t' , 'tcp_po' );?></a>
				</div>
			<?php else: ?>
				<div class="wrap delete_children" id="div_delete_children_<?php echo $option->id_to;?>" style="display: none; border: 1px dotted orange; padding: 2px">
					...
					<input id="rel_type" name="rel_type" value="<?php echo $rel_type;?>" type="hidden" />
					<p><?php _e( 'Do you really want to delete those children?', 'tcp_po' );?></p>
					<input type="submit" id="tcp_delete_children" name="tcp_delete_children_<?php echo $i;?>" class="button-secondary" value="<?php _e( 'Yes' , 'tcp_po' );?>"/> |
					<a href="#" onclick="jQuery('#div_delete_children_<?php echo $option->id_to;?>').hide();return false;"><?php _e( 'No, I don\'t' , 'tcp_po' );?></a>
				</div>
			<?php endif;?>
		</td>
	</tr>
	...
	<tr>
		<td style="padding-left: 2em;"><a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post_2->ID;?>" title="<?php _e( 'edit option', 'tcp_po' ); ?>"><?php echo tcp_get_the_thumbnail( $post_id, $option->id_to, $post_2->ID, array( '50', '50' ) ); ?></a></td>
		<td><span style="padding-left: 2em;">&nbsp;</span><a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post_2->ID;?>" title="<?php _e( 'edit option', 'tcp_po' ); ?>"><?php echo $post_2->post_title;?></a></td>
		...
		<td>
			...
			&nbsp;<label><?php echo __( 'Order', 'tcp_po' );?>:&nbsp;<input type="text" id="tcp_order" name="tcp_order_<?php echo $i, '_', $j;?>" value="<?php echo tcp_get_the_order( $post_2->ID );?>" size="4" maxlength="8" class="tcp_count"/></label>
		</td>
		<td>
			<a href="post.php?action=edit&post_type=tcp_product_option&post=<?php echo $post_2->ID;?>"><?php _e( 'edit option', 'tcp_po' );?></a>
			<?php if ( RelEntities::count( $option_2->id_to, $rel_type ) == 0 ) :?>
				&nbsp;|&nbsp;
				<a href="#" onclick="return show_delete_relation(<?php echo $option_2->id_to;?>);" class="delete"><?php _e( 'delete option', 'tcp_po' );?></a>
				<div class="wrap delete_relation" id="div_delete_relation_<?php echo $option_2->id_to;?>" style="display: none; border: 1px dotted orange; padding: 2px">
					...
					<p><?php _e( 'Do you really want to delete the option?', 'tcp_po' );?></p>
					<input type="submit" id="tcp_delete_relation" name="tcp_delete_relation_<?php echo $i, '_', $j;?>" class="button-secondary" value="<?php _e( 'Yes' , 'tcp_po' );?>"/> |
					<a href="#" onclick="jQuery('#div_delete_relation_<?php echo $option_2->id_to;?>').hide();return false;"><?php _e( 'No, I don\'t' , 'tcp_po' );?></a>
				</div>
			<?php endif;?>
		</td>
	</tr>
...
<?php else:?>
	<tr>
		<td colspan="6"><?php _e( 'The options list is empty', 'tcp_po' )?></td>
	</tr>
<?php endif;?>
</tbody>
</table>
